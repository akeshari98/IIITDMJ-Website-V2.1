...
const NewsPage = () => {
  ...
  const filteredAndSortedNews = data
    .filter(news => 
      ...
      news.createdAt.toLowerCase().includes(searchTerm.toLowerCase())
    )
    .sort((a, b) => {
  ...
  const crumbs = [{crumb: "Notices", link:"#"}];

  return (
    <div>
      <PageHeader breadCrumbs={crumbs} title={"Notices"}/>
        ...
        <div className="max-w-7xl mx-auto">
            ...
            {filteredAndSortedNews.map((news) => (
              <article key={news.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                {/* <div className="aspect-w-16 aspect-h-9 relative">
                  <img
                    src={news.image_url ? news.image_url : newsPlaceHolder}
                    alt={news.title}
                ...
                <div className="p-6">
                 ...
                 {news.link && <div className="mt-4 pt-4 border-t border-gray-100">
                    <a
                      href={news.link}
                      target="_blank"
          ...
          {filteredAndSortedNews.length === 0 && (
            <div className="text-center py-12">
              <p className="text-gray-600 text-lg">No Notices found matching your search.</p>
            </div>
          )}
        </div>
      </div>
