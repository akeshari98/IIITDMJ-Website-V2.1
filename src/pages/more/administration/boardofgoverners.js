...
const MainPage = () => {
  ...
  useEffect(() => {
    const endpoints = [{ key: "cardsData", endpoint: "boardofgoverners" }];
  ...
  const downloads = [
    ...
    {
      name: "BOG Meetings Renumbered",
      href: "https://www.iiitdmj.ac.in/administration/downloads/BOG%20meetings%20renumbered%20list%2012-11-13.pdf",
    },
  ];
  const renderLink = (item) =>
    item.href.startsWith("/") ? (
      <Link to={item.href} className="text-blue-500 no-underline">
        <span>{item.name}</span>
      </Link>
    ) : (
      <a
