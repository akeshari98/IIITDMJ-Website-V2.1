...
const getAllFaculty = async (req, res) => {
  try {
    const query = `
      ...
      FROM 
          dblink('dbname=${process.env.Fusion_DB_NAME} user=${process.env.Fusion_DB_USER} password=${process.env.Fusion_DB_PASSWORD}', 
              'SELECT auth_user.id, user_type, first_name, last_name, email, address, phone_no, profile_picture FROM auth_user, globals_extrainfo, eis_faculty_about 
              WHERE auth_user.id=globals_extrainfo.user_id 
              AND auth_user.id=eis_faculty_about.user_id') AS t(id int, user_type varchar, first_name varchar, last_name varchar, email varchar, address text, phone_no bigint, profile_picture varchar)
    `;
    // console.log(res);
