...
async function getFacultyBasicInfo(userId) {
  const query = `
    SELECT id, first_name, last_name, designation, email, address, profile_picture, department
    FROM 
      dblink('dbname=fusionlab user=superAdmin password=9455957884', 
             ...
             $$
          ) AS t(id int, first_name varchar, last_name varchar, designation varchar, email varchar, address text, profile_picture varchar, department varchar);
  `;
      ...
      dblink('dbname=fusionlab user=superAdmin password=9455957884', 
             'SELECT auth_user.id AS id, 
              ...
              WHERE extra_info.department_id = ${branch_id} 
                 AND designation.id IN (1, 2, 3, 14)
            ') AS t(id int, first_name varchar, last_name varchar, designation varchar, email varchar, address text, profile_picture varchar, department varchar);
  `;
