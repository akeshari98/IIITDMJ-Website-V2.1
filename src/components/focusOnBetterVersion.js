...
const FocusOn = () => {
  ...
  return (
    <div className="w-full overflow-x-auto md:overflow-x-visible border rounded-xl bg-transparent">
      <div  className="flex flex-row md:flex-row space-x-2 min-w-max md:min-w-0 transform-gpu bg-transparent" // Add transform-gpu
    style={{ willChange: 'transform' }} >
        {cards.map((card, index) => (
          <motion.a 
            key={index}
            href={card.link}
            className="relative w-64 h-96 flex-shrink-0  md:w-[22vw] group overflow-visible font-sans rounded-xl"
            whileHover="hover"
            initial="initial"
            variants={{
