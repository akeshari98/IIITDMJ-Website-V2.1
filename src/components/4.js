...
const FocusCard = ({ title, icon: Icon, link, bgImage }) => (
  <a href={link} className="block aspect-square">
    <div className="relative h-full group overflow-hidden rounded-xl border border-gray-300">
  ...
  />
      ...
      {/* Content */}
      <div className="relative h-full p-6 flex flex-col items-center justify-center text-center space-y-4 bg-opacity-90 bg-gradient-to-t from-gray-900 to-transparent  transition-all duration-100">
        <div className="p-3 rounded-full bg-white/10 backdrop-blur-sm group-hover:bg-blue-50 transition-all duration-100">
          <Icon className="w-8 h-8 text-white group-hover:text-blue-600 transition-colors duration-100" />
        </div>
        
        <h3 className="font-medium text-lg text-white group-hover:text-blue-50 transition-colors duration-100">
          {title}
        </h3>
      </div>
    </div>
